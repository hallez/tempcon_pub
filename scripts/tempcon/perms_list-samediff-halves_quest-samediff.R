...
for (iperm in 1:numperm){
  qsd_halves_permute <- z_mm_filt %>%
    ...
    dplyr::mutate(mod_hemi_perm = purrr::map(by_roi, ~lme4::lmer(z_r ~ hemi + (1 | subj), data = .,
                  ...
                  mod_list_halves_perm = purrr::map(by_roi, ~lme4::lmer(z_r ~ hemi + list_halves_perm + (1 | subj), data = .,
                                                                        REML = FALSE, lme4::lmerControl(calc.derivs = FALSE))),
                  mod_quest_same_diff_perm = purrr::map(by_roi, ~lme4::lmer(z_r ~ hemi + question_perm + (1 | subj), data = .,
                                                                        REML = FALSE, lme4::lmerControl(calc.derivs = FALSE))),
                  mod_list_halves_same_diff_perm = purrr::map(by_roi, ~lme4::lmer(z_r ~ hemi + list_halves_perm + question_perm + (1 | subj), data = .,
                                                                                 REML = FALSE, lme4::lmerControl(calc.derivs = FALSE))),
                  mod_list_halvesXsame_diff_perm = purrr::map(by_roi, ~lme4::lmer(z_r ~ hemi + (list_halves_perm * question_perm) + (1 | subj), data = .,
                                                                                 REML = FALSE, lme4::lmerControl(calc.derivs = FALSE))),
  ...
  progress_fname <- file.path(analyzed_mri_dir,
                              "perms_list-samediff-halves_quest-samediff_progress.csv")
...
orig_counts <- z_mm_filt %>%
  dplyr::filter(list_lag >= 0,
                question %in% c("same", "diff"),
                list_same_diff_half %in% c("same_half", "diff_half")) %>%
  dplyr::group_by(list_same_diff_half, question) %>%
  dplyr::tally() %>%
  dplyr::mutate(data_type = "original")

perm_counts <- NULL
perm_counts <- qsd_halves_permute %>%
  dplyr::select(roi, by_roi) %>%
  tidyr::unnest() %>%
  dplyr::group_by(list_halves_perm, question_perm) %>%
  dplyr::tally() %>%
  dplyr::mutate(data_type = "permuted") %>%
  # rename columns so that plays nicely when join dataframes
  dplyr::rename("list_same_diff_half" = "list_halves_perm", "question" = "question_perm") %>%
...
orig_counts %>%
  ...
  dplyr::full_join(perm_counts, by = c("n", "data_type", "list_same_diff_half", "question")) %>%
...
if(SAVE_GRAPH_FLAG == 1){
  ggplot2::ggsave(filename = file.path(graph_out_dir, "list-same-diff-halves_quest-same-diff_perm-trialnums.pdf"),
                  height = 6, width = 12)
}
...
qsd_halves_computed_pvals <- list_sdhalves_quest_samediff_stats %>%
  dplyr::left_join(., qsd_halves_permute_anova, by = c("roi")) %>%
...
same_diff_list_halves_all_results <- list_sdhalves_quest_samediff_stats %>%
  dplyr::left_join(., qsd_halves_permute_anova, by = "roi") %>%
  dplyr::left_join(., qsd_halves_permuted_pvals, by = c("roi", "nperm", "chisq_ME.listhalves_perm",
...
if(SAVE_GRAPH_FLAG == 1){
  ggplot2::ggsave(filename = file.path(graph_out_dir, "list-same-diff-halves_quest-same-diff_ME-list.pdf"),
                  height = 6, width = 12)
}

#' ## Plot permuted distribution: ME question (same/diff)
plot_permutation(same_diff_list_halves_all_results, "chisq_ME.quest_perm", "chisq_ME.quest_perm_p95", "chisq_ME.quest", "roi")
print(p)

if(SAVE_GRAPH_FLAG == 1){
  ggplot2::ggsave(filename = file.path(graph_out_dir, "list-same-diff-halves_quest-same-diff_ME-question.pdf"),
                  height = 6, width = 12)
}

#' ## Plot permuted distribution: list same/diff half and ME question vs list lag and question (same/diff) interaction
plot_permutation(same_diff_list_halves_all_results, "chisq_listhalvessamediff.listhalvesXsamediff_perm", "chisq_listhalvessamediff.listhalvesXsamediff_perm_p95", "chisq_listhalvesquest.listhalvesXquest", "roi")
print(p)

if(SAVE_GRAPH_FLAG == 1){
  ggplot2::ggsave(filename = file.path(graph_out_dir, "list-same-diff-halves_quest-same-diff_list-same-diff-halves-ME-quest-same-diff-vs-interaction.pdf"),
                  height = 6, width = 12)
}
