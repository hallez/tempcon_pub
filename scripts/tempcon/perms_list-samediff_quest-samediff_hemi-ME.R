...
for (iperm in 1:numperm){
  ...
  progress_fname <- file.path(analyzed_mri_dir,
                              "perms_list-samediff_quest-samediff_hemi-ME_progress.csv")
...
orig_counts <- z_mm_filt %>%
  dplyr::filter(list_lag >= 0,
                question %in% c("same", "diff"),
                list_same_diff %in% c("same_list", "diff_list")) %>%
  dplyr::group_by(list_same_diff, question, hemi) %>%
  dplyr::tally() %>%
  dplyr::mutate(data_type = "original")

perm_counts <- NULL
perm_counts <- qsd_lsd_hemi_permute %>%
  dplyr::select(roi, by_roi) %>%
  tidyr::unnest() %>%
  dplyr::group_by(hemi_perm, list_same_diff, question) %>%
  dplyr::tally() %>%
...
orig_counts %>%
  ...
  dplyr::full_join(perm_counts, by = c("n", "data_type", "list_same_diff", "question", "hemi")) %>%
...
if(SAVE_GRAPH_FLAG == 1){
  ggplot2::ggsave(filename = file.path(graph_out_dir, "list-same-diff_quest-same-diff_hemi-ME_perm-trialnums.pdf"),
                  height = 6, width = 12)
}

#' ### Compute permuted p-values
qsd_lsd_permuted_pvals <- qsd_lsd_hemi_permute_anova %>%
  dplyr::group_by(roi) %>%
...
same_diff_list_sd_all_results <- list_samediff_quest_samediff_stats %>%
  dplyr::left_join(., qsd_lsd_hemi_permute_anova, by = c("roi")) %>%
  dplyr::left_join(., qsd_lsd_permuted_pvals, by = c("roi", "nperm", "chisq_null.hemi_perm")) %>%
...
save("same_diff_list_sd_truncated", file = file.path(analyzed_mri_dir, "perms_list-samediff_quest-samediff_hemi-ME_chisq-perms.RData"))

#' ## Plot permuted distribution: Null vs hemi
plot_permutation(same_diff_list_sd_all_results, "chisq_null.hemi_perm", "p95", "chisq_null.hemi", "roi")
print(p)

if(SAVE_GRAPH_FLAG == 1){
  ggplot2::ggsave(filename = file.path(graph_out_dir, "list-same-diff_quest-same-diff_null-vs-hemi.pdf"),
                  height = 6, width = 12)
}
