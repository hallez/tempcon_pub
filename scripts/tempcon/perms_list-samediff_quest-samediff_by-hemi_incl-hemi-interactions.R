...
qsd_lsd_hemi_permute_anova <- data.frame(stringsAsFactors = FALSE)
for (iperm in 1:numperm){
  qsd_lsd_hemi_permute <- z_mm_filt %>%
    ...
    tidyr::unite(listsd_qsamediff_hemi, list_same_diff, question, hemi, sep = ".") %>%
    tidyr::nest(.key = "trials") %>%
    dplyr::mutate(listsd_qsamediff_hemi_perm = purrr::map(trials, ~sample(x = .$listsd_qsamediff_hemi, size = length(.$listsd_qsamediff_hemi),
                                                                         replace = FALSE))) %>%
    tidyr::unnest() %>%
    # use the `\\` to escape the period so it's read as a literal period (rather than regex)
    tidyr::separate(listsd_qsamediff_hemi_perm, into = c("list_sd_perm", "question_perm", "hemi_perm"), sep = "\\.") %>%
    dplyr::group_by(roi) %>%
    tidyr::nest(.key = "by_roi") %>%
    dplyr::mutate(mod_list_sdXquest_perm = purrr::map(by_roi, ~lme4::lmer(z_r ~ hemi_perm + (list_sd_perm * question_perm) + (1 | subj), data = .,
                                                                         REML = FALSE, lme4::lmerControl(calc.derivs = FALSE))),
                  mod_list_sdXquestXhemi_perm = purrr::map(by_roi, ~lme4::lmer(z_r ~ (list_sd_perm * question_perm * hemi_perm) + (1 | subj), data = .,
                                                                              REML = FALSE, lme4::lmerControl(calc.derivs = FALSE))),
                  aov_listsdXquest.listsdXquestXhemi_perm = purrr::map2(mod_list_sdXquest_perm, mod_list_sdXquestXhemi_perm, anova),
                  chisq_listsdXquest.listsdXquestXhemi_perm = purrr::map_dbl(aov_listsdXquest.listsdXquestXhemi_perm, ~.$Chisq[2]),
                  nperm = as.numeric(iperm))

  print(message("permutation", as.numeric(iperm)))

  if(dim(qsd_lsd_hemi_permute_anova)[1] == 0){
    qsd_lsd_hemi_permute_anova <- qsd_lsd_hemi_permute %>%
      dplyr::select(nperm, roi, starts_with("chisq"))
  } else {
    cur_dat <- qsd_lsd_hemi_permute %>%
      dplyr::select(nperm, roi, starts_with("chisq"))

    qsd_lsd_hemi_permute_anova <- dplyr::bind_rows(qsd_lsd_hemi_permute_anova, cur_dat)
  }

  # write out progress to a file so that know script is running
  progress_fname <- file.path(analyzed_mri_dir,
                              "perms_list-samediff_quest-samediff-hemi_interactions_progress.csv")
  write.csv(data.frame(perm_iteration = unique(qsd_lsd_hemi_permute_anova$nperm)), file = progress_fname, append = TRUE, quote = FALSE, row.names = FALSE)

}

#' ## Compute permuted p-values
qsd_lsd_hemi_permuted_pvals <- qsd_lsd_hemi_permute_anova %>%
  dplyr::group_by(roi) %>%
...
qsd_lsd_hemi_computed_pvals <- list_samediff_quest_samediff_stats %>%
  dplyr::left_join(., qsd_lsd_hemi_permute_anova, by = c("roi")) %>%
  dplyr::mutate(listsdXsamediff.listsdXsamediffXhemi_perm_exceeds = case_when(chisq_listsdXquest.listsdXquestXhemi_perm > chisq_listsdXquest.listsdXquestXhemi ~ 1,
                                                                                      chisq_listsdXquest.listsdXquestXhemi_perm <= chisq_listsdXquest.listsdXquestXhemi ~ 0)) %>%
  dplyr::group_by(roi) %>%
  dplyr::summarise(chisq_listsdXquest.listsdXquestXhemi_num_signif_perms = sum(listsdXsamediff.listsdXsamediffXhemi_perm_exceeds)) %>%
  dplyr::mutate(chisq_listsdXquest.listsdXquestXhemi_computed_pval = (chisq_listsdXquest.listsdXquestXhemi_num_signif_perms + 1) / (numperm + 1))

#' ## Pretty print stat results
same_diff_list_sd_hemi_all_results <- list_samediff_quest_samediff_stats %>%
  dplyr::left_join(., qsd_lsd_hemi_permute_anova, by = c("roi")) %>%
  dplyr::left_join(., qsd_lsd_hemi_permuted_pvals, by = c("roi", "nperm", "chisq_listsdXquest.listsdXquestXhemi_perm")) %>%
  dplyr::left_join(., qsd_lsd_hemi_computed_pvals, by = c("roi"))

same_diff_list_sd_hemi_all_results %>%
  dplyr::select(roi, contains("computed_pval"),
                "chisq_listsdXquest.listsdXquestXhemi") %>%
  dplyr::distinct() %>%
  dplyr::select(roi,
                chisq_listsdXquest.listsdXquestXhemi, chisq_listsdXquest.listsdXquestXhemi_computed_pval) %>%
  knitr::kable() %>%
  kableExtra::kable_styling(bootstrap_options = "striped")

#' ### Save out permutations for future use
same_diff_list_sd_hemi_truncated <- same_diff_list_sd_hemi_all_results %>%
  dplyr::select(-starts_with("mod"), -starts_with("aov"), -by_roi)
save("same_diff_list_sd_hemi_truncated", file = file.path(analyzed_mri_dir, "perms_list-samediff_quest-samediff-hemi_interactions_chisq-perms.RData"))
