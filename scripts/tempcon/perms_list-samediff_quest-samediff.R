...
for (iperm in 1:numperm){
  qsd_lsd_permute <- z_mm_filt %>%
    ...
    dplyr::mutate(mod_hemi_perm = purrr::map(by_roi, ~lme4::lmer(z_r ~ hemi + (1 | subj), data = .,
                                                                 REML = FALSE, lme4::lmerControl(calc.derivs = FALSE))),
                  mod_list_sd_perm = purrr::map(by_roi, ~lme4::lmer(z_r ~ hemi + list_sd_perm + (1 | subj), data = .,
                                                                        REML = FALSE, lme4::lmerControl(calc.derivs = FALSE))),
                  mod_quest_same_diff_perm = purrr::map(by_roi, ~lme4::lmer(z_r ~ hemi + question_perm + (1 | subj), data = .,
                                                                        REML = FALSE, lme4::lmerControl(calc.derivs = FALSE))),
                  mod_list_sd_same_diff_perm = purrr::map(by_roi, ~lme4::lmer(z_r ~ hemi + list_sd_perm + question_perm + (1 | subj), data = .,
                                                                                 REML = FALSE, lme4::lmerControl(calc.derivs = FALSE))),
                  mod_list_sdXsame_diff_perm = purrr::map(by_roi, ~lme4::lmer(z_r ~ hemi + (list_sd_perm * question_perm) + (1 | subj), data = .,
                                                                                 REML = FALSE, lme4::lmerControl(calc.derivs = FALSE))),
  ...
  progress_fname <- file.path(analyzed_mri_dir,
                              "perms_list-samediff_quest-samediff_progress.csv")
...
orig_counts <- z_mm_filt %>%
  dplyr::filter(list_lag >= 0,
                question %in% c("same", "diff"),
                list_same_diff %in% c("same_list", "diff_list")) %>%
  dplyr::group_by(list_same_diff, question) %>%
  dplyr::tally() %>%
  dplyr::mutate(data_type = "original")

perm_counts <- NULL
perm_counts <- qsd_lsd_permute %>%
  dplyr::select(roi, by_roi) %>%
  tidyr::unnest() %>%
  dplyr::group_by(list_sd_perm, question_perm) %>%
  dplyr::tally() %>%
  dplyr::mutate(data_type = "permuted") %>%
  # rename columns so that plays nicely when join dataframes
  dplyr::rename("list_same_diff" = "list_sd_perm", "question" = "question_perm") %>%
  # actually make `list_same_diff` a factor so don't get warning when merge dataframes
  dplyr::ungroup() %>%
  dplyr::mutate(list_same_diff = factor(list_same_diff))

orig_counts %>%
  # NB: will get an error about `list_same_diff` and `question` having different levels - OK to ignore
  dplyr::full_join(perm_counts, by = c("n", "data_type", "list_same_diff", "question")) %>%
  dplyr::group_by(data_type) %>%
  ggplot2::ggplot(ggplot2::aes(x = list_same_diff, y = n, fill = data_type)) +
...
if(SAVE_GRAPH_FLAG == 1){
  ggplot2::ggsave(filename = file.path(graph_out_dir, "list-same-diff_quest-same-diff_perm-trialnums.pdf"),
                  height = 6, width = 12)
}
...
qsd_lsd_computed_pvals <- list_samediff_quest_samediff_stats %>%
  dplyr::left_join(., qsd_lsd_permute_anova, by = c("roi")) %>%
...
same_diff_list_sd_all_results <- list_samediff_quest_samediff_stats %>%
  dplyr::left_join(., qsd_lsd_permute_anova, by = "roi") %>%
  dplyr::left_join(., qsd_lsd_permuted_pvals, by = c("roi", "nperm", "chisq_ME.listsd_perm",
                                                       "chisq_ME.quest_perm", "chisq_listsdsamediff.listsdXsamediff_perm")) %>%
  dplyr::left_join(., qsd_lsd_computed_pvals, by = "roi")

same_diff_list_sd_all_results %>%
...
save("same_diff_list_sd_truncated", file = file.path(analyzed_mri_dir, "perms_list-samediff_quest-samediff_chisq-perms.RData"))

#' ## Plot permuted distribution: ME list same/diff half
plot_permutation(same_diff_list_sd_all_results, "chisq_ME.listsd_perm", "chisq_ME.listsd_perm_p95", "chisq_ME.listsd", "roi")
print(p)

if(SAVE_GRAPH_FLAG == 1){
  ggplot2::ggsave(filename = file.path(graph_out_dir, "list-same-diff_quest-same-diff_ME-list.pdf"),
                  height = 6, width = 12)
}

#' ## Plot permuted distribution: ME question (same/diff)
plot_permutation(same_diff_list_sd_all_results, "chisq_ME.quest_perm", "chisq_ME.quest_perm_p95", "chisq_ME.quest", "roi")
print(p)

if(SAVE_GRAPH_FLAG == 1){
  ggplot2::ggsave(filename = file.path(graph_out_dir, "list-same-diff_quest-same-diff_ME-question.pdf"),
                  height = 6, width = 12)
}

#' ## Plot permuted distribution: list same/diff half and ME question vs list lag and question (same/diff) interaction
plot_permutation(same_diff_list_sd_all_results, "chisq_listsdsamediff.listsdXsamediff_perm", "chisq_listsdsamediff.listsdXsamediff_perm_p95", "chisq_listsdquest.listsdXquest", "roi")
print(p)

if(SAVE_GRAPH_FLAG == 1){
  ggplot2::ggsave(filename = file.path(graph_out_dir, "list-same-diff_quest-same-diff_list-same-diff-ME-quest-same-diff-vs-interaction.pdf"),
                  height = 6, width = 12)
}
